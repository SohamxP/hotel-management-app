...
export default function Dashboard() {
  ...
  const reserved = rooms.filter((r) => r.AvailStatus === "Reserved").length;
  const occupied = rooms.filter((r) => r.AvailStatus === "Occupied").length;
  const blocked = rooms.filter((r) => r.AvailStatus === "Blocked").length;

  const filteredRooms = rooms.filter((room) => {
    const matchesFilter = filter === "All" || room.AvailStatus === filter;
  ...
  <Text style={{ color: COLORS.text, textAlign: "center", fontWeight: "800" }}>
    Logout
      ...
      <View style={{ flexDirection: "row", gap: 8, marginTop: 20 }}>
        <StatCard title="Total" value={rooms.length} />
        <StatCard title="Available" value={available} color={COLORS.success} />
        <StatCard title="Reserved" value={reserved} color={COLORS.warning} />
        <StatCard title="Occupied" value={occupied} color={COLORS.primary} />
      ...
      <View style={{ flexDirection: "row", gap: 8, marginVertical: 20 }}>
        {["All", "Available", "Reserved", "Occupied", "Blocked"].map((item) => (
          ...
          >
            <Text
              style={{
                color: filter === item ? "#00111A" : COLORS.text,
                fontWeight: "700",
              }}
            >
              {item}
          ...
          renderItem={({ item }) => (
            ...
            >
              ...
              <Text style={{ color: COLORS.muted, marginTop: 8 }}>
                {item.RoomType} • ${item.RatePerNight}/night
              </Text>

              <Text style={{ color: COLORS.muted, marginTop: 4 }}>
                Max {item.MaxOccupancy} guests • WiFi {item.HasWifi} • TV {item.HasTv}
              </Text>
            </Pressable>
          )}
        />
